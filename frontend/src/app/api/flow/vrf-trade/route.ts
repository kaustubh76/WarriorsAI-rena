...
export const POST = composeMiddleware([
  withRateLimit({ prefix: 'flow-vrf-trade', ...RateLimitPresets.storageWrite }),
  async (req, ctx) => {
    const timer = new PerformanceTimer('vrf_trade');

    try {
      const body: VRFTradeRequest = await req.json();
      const {
        mirrorKey,
        agentId,
        isYes,
      ...
      throw error; // Re-throw to let composeMiddleware handle it
    }
  },
], { errorContext: 'API:Flow:VRFTrade:POST' });

/**
 * GET: Get trade status or mirror market info
 */
export const GET = composeMiddleware([
  withRateLimit({ prefix: 'flow-vrf-trade-get', ...RateLimitPresets.apiQueries }),
  async (req, ctx) => {
...
], { errorContext: 'API:Flow:VRFTrade:GET' });
