...
export const GET = composeMiddleware([
  withRateLimit({ prefix: 'flow-scheduled', ...RateLimitPresets.apiQueries }),
  async (req, ctx) => {
    try {
      if (!CONTRACT_ADDRESS) {
...
], { errorContext: 'API:Flow:Scheduled:GET' });
...
], { errorContext: 'API:Flow:Scheduled:POST' });
...
], { errorContext: 'API:Flow:Scheduled:PUT' });
...
], { errorContext: 'API:Flow:Scheduled:DELETE' });
