...
import { CreatorRevenueCard, TierProgressCard, ClaimRewardsButton, CreatorProfileCard } from '@/components/creator';
...
export default function CreatorDashboardPage() {
  ...
  const { totalCreators, totalFeesDistributedFormatted, loading: globalLoading } = useGlobalCreatorStats();
      ...
      <main className="container-arcade py-8 md:py-12">
        <div className="text-center animate-fade-in">
              ...
              {tiers.map((tier, index) => (
                ...
                >
                  <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-lg md:text-xl flex-shrink-0 ${
                    tier.tier === 0 ? 'bg-orange-500/20 border border-orange-500/30' :
                    tier.tier === 1 ? 'bg-slate-400/20 border border-slate-400/30' :
                    tier.tier === 2 ? 'bg-arcade-gold/20 border border-arcade-gold/30' :
                    tier.tier === 3 ? 'bg-cyan-500/20 border border-cyan-500/30' :
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/create-market" className="btn btn-primary">
              Create Market
            </Link>
  ...
  return (
    <main className="container-arcade py-6 md:py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
          ...
          <div className="card animate-slide-up" style={{ animationDelay: '150ms' }}>
            ...
            {history.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-400 text-sm mb-2">No revenue history yet</p>
                <p className="text-slate-500 text-xs mb-4">Create markets or mint warriors to start earning fees</p>
                <Link
                  href="/create-market"
                  className="inline-block px-4 py-2 text-xs bg-purple-600/20 hover:bg-purple-600/30 text-purple-400 rounded-lg transition-colors"
                >
        ...
        {/* Right Column - Profile, Tier & Claim */}
        <div className="space-y-4 md:space-y-6">
          <div className="animate-slide-up" style={{ animationDelay: '200ms' }}>
            {creator && <CreatorProfileCard creator={creator} />}
          </div>
          <div className="animate-slide-up" style={{ animationDelay: '250ms' }}>
            {creator && <TierProgressCard creator={creator} />}
          </div>
          <div className="animate-slide-up" style={{ animationDelay: '300ms' }}>
            <ClaimRewardsButton onSuccess={refetch} />
          </div>
        </div>
      </div>
      ...
      {stats && (
        <div className="mt-6 md:mt-8 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 animate-fade-in" style={{ animationDelay: '300ms' }}>
          <StatCard label="Total Earnings" value={`${parseFloat(formatEther(stats.totalEarnings)).toFixed(2)} CRwN`} icon="💰" />
          <StatCard label="Weekly Earnings" value={`${parseFloat(formatEther(stats.weeklyEarnings)).toFixed(2)} CRwN`} icon="📅" />
          <StatCard label="Monthly Earnings" value={`${parseFloat(formatEther(stats.monthlyEarnings)).toFixed(2)} CRwN`} icon="📊" />
